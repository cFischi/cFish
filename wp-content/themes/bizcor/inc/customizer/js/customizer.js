( function( api ) {
    ...
    api.sectionConstructor['bizcor-pro'] = api.Section.extend( {
		...
		_init: function() {
            ...
            control.actions = function ($context) {
                if (control.params.live_title_id) {
                    ...
                    if (control.id === 'bizcor_section_order_styling') {
    ...
    function _the_editor( container ){
        var _editor = {
            ...
            ready: function( container ) {
                ...
                control.container.addClass( 'bizcor-editor-added' );
...
jQuery( window ).ready( function( $ ){
    ...
    if ( typeof bizcor_customizer_settings !== "undefined" ) {
        if (bizcor_customizer_settings.number_action > 0) {
            $('.control-section-themes h3.accordion-section-title').append('<a class="theme-action-count" href="' + bizcor_customizer_settings.action_url + '">' + bizcor_customizer_settings.number_action + '</a>');
        }
    }
     ...
     */
    $( 'select[data-customize-setting-link="bizcor_hero_layout"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';

        $( "li[id^='customize-control-bizcor_hcl']").hide();
        $( "li[id^='customize-control-bizcor_hcl"+v+"']").show();

    } );

    $( 'select[data-customize-setting-link="bizcor_hero_layout"]').trigger( 'on_custom_load' );

    /**
     * For Gallery content settings
     */
    $( 'select[data-customize-setting-link="bizcor_gallery_source"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';

        $( "li[id^='customize-control-bizcor_gallery_source_']").hide();
        $( "li[id^='customize-control-bizcor_gallery_api_']").hide();
        $( "li[id^='customize-control-bizcor_gallery_settings_']").hide();
        $( "li[id^='customize-control-bizcor_gallery_source_"+v+"']").show();
        $( "li[id^='customize-control-bizcor_gallery_api_"+v+"']").show();
        $( "li[id^='customize-control-bizcor_gallery_settings_"+v+"']").show();

    } );

    $( 'select[data-customize-setting-link="bizcor_gallery_source"]').trigger( 'on_custom_load' );

    /**
     * For Gallery display settings
     */
    $( 'select[data-customize-setting-link="bizcor_gallery_display"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';
        switch ( v ) {
            case 'slider':
                $( "#customize-control-bizcor_g_row_height, #customize-control-bizcor_g_col, #customize-control-bizcor_g_spacing").hide();
                break;
            case 'justified':
                $( "#customize-control-bizcor_g_col, #customize-control-bizcor_g_spacing").hide();
                $( "#customize-control-bizcor_g_row_height").show();
                break;
            case 'carousel':
                $( "#customize-control-bizcor_g_row_height, #customize-control-bizcor_g_col").hide();
                $( "#customize-control-bizcor_g_col, #customize-control-bizcor_g_spacing").show();
                break;
            case 'masonry':
                $( "#customize-control-bizcor_g_row_height").hide();
                $( "#customize-control-bizcor_g_col, #customize-control-bizcor_g_spacing").show();
                break;
            default:
                $( "#customize-control-bizcor_g_row_height").hide();
    ...
    $( 'select[data-customize-setting-link="bizcor_gallery_display"]').trigger( 'on_custom_load' );
...
jQuery( document ).ready( function( $ ) {
    ...
    display_footer_layout( $( '#customize-control-bizcor_footer_middle_columns select' ).val() );
    $( '#customize-control-bizcor_footer_middle_columns select' ).on( 'change', function ()  {
