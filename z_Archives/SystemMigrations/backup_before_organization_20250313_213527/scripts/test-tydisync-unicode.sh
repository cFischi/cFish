...
#
# Shell script wrapper for the Unicode character testing script for tYDiSync~
...
LOG_DIR="../logs"
LOG_FILE="$LOG_DIR/unicode-test-output.log"
...
# Print header
echo -e "${YELLOW}==================================================================${NC}"
echo -e "${YELLOW}                   tYDiSync~ Unicode Test                         ${NC}"
echo -e "${YELLOW}==================================================================${NC}"
echo

# Check if the test script exists
TEST_SCRIPT="$SCRIPT_DIR/test-tydisync-unicode.js"
...
# Run the test
echo -e "${YELLOW}Starting Unicode character test...${NC}"
...
if [ ${PIPESTATUS[0]} -eq 0 ]; then
    echo -e "\n${GREEN}Unicode test completed successfully!${NC}"
    echo "See detailed results in the logs directory."
    exit 0
else
    echo -e "\n${RED}Unicode test failed!${NC}"
