	...
	function um_terms_conditions_check_dependencies() {
		if ( ! defined( 'um_path' ) || ! file_exists( um_path  . 'includes/class-dependencies.php' ) ) {
			...
			function um_terms_conditions_dependencies() {
				// translators: %s is the Terms&Conditions extension name.
...
if ( ! function_exists( 'um_terms_conditions_activation_hook' ) ) {
	function um_terms_conditions_activation_hook() {
		...
		//run setup
		if ( ! class_exists( 'um_ext\um_terms_conditions\core\Setup' ) ) {
			require_once um_terms_conditions_path . 'includes/core/class-setup.php';
		}

		$setup = new um_ext\um_terms_conditions\core\Setup();
		$setup->run_setup();
	}
}
