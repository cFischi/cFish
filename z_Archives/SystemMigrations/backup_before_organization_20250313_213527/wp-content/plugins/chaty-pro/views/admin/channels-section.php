...
<section class="one chaty-setting-form" id="<?php echo esc_attr($pro_id) ?>" style="padding-bottom: 0;" xmlns="http://www.w3.org/1999/html">
    ...
    <?php
    $social_icons = $icons = get_option('cht_numb_slug'.$this->widget_index, false);
    $addChatway = is_plugin_active('chatway-live-chat/chatway.php') ? true : false;
    $isChatwayAdded = get_option('cht_is_chatway_added', false);

    $social_icons = ($social_icons == false) ? "whatsapp" : $social_icons;
    $social_icons = trim($social_icons, ",");
    $social_icons = explode(",", $social_icons);
    $social_icons = array_unique($social_icons);
    if((isset($_GET['chaty_action']) && $_GET['chaty_action'] == 'add_chatway') || ($this->widget_index != '_new_widget' && $addChatway && !$isChatwayAdded)) {
        if(!in_array('Chatway', $social_icons)) {
            array_unshift($social_icons, "Chatway");
        }
    }
    ...
    <div class="channels-icons" id="channel-list">
        <?php if ($this->socials) :
            foreach ($this->socials as $key => $social) :
                ...
                $active_class = '';
                foreach ($social_icons as $key_soc) :
    ...
    <div class="social-channels mt-5">
